...
pub fn main(process: std.process.Init.Minimal) !void {
    ...
    // What a per-frame ring costs here and how many of them one descriptor set
    // can carry. The ring pads each frame up to the offset alignment and binds
    // one descriptor for every frame, so the first pair is the padding and the
    // second is the budget that decides how many rings can work that way.
    const limits = context.properties.limits;
    std.log.info("limits: dynamic offset alignment, uniform {d}, storage {d}", .{
        limits.min_uniform_buffer_offset_alignment,
        limits.min_storage_buffer_offset_alignment,
    });
    std.log.info("limits: dynamic bindings per set, uniform {d}, storage {d}", .{
    ...
    // A per-frame ring on the device. Two frames, because one shows no stride
    // at all, and the whole point of the single-buffer form is that the frames
    // are separated by an offset rather than by being different buffers.
    const ring_frames = 2;
    const CameraUbo = extern struct {
        view_projection: [16]f32 align(16),
        position: [4]f32 align(16),
    };
    var camera_ring: gpu.PerFrame(CameraUbo) = try .init(
        &context,
        &allocator,
        ring_frames,
        1,
    ...
    defer camera_ring.deinit();

    for (0..ring_frames) |index|
        camera_ring.slice(index)[0].position = .{ @floatFromInt(index), 0, 0, 1 };

    // Read back through the same mapping. Slots that overlap would return the
    // last value written for both, and nothing else reports that: a validation
    // layer sees a legal buffer and a legal offset either way.
    var separated = true;
    for (0..ring_frames) |index|
        separated = separated and
            camera_ring.slice(index)[0].position[0] == @as(f32, @floatFromInt(index));

    std.log.info("per-frame ring: payload {d} B, stride {d} B, offsets {d} and {d}, separated {}", .{
        @sizeOf(CameraUbo),
        camera_ring.stride,
        camera_ring.dynamicOffset(0),
        camera_ring.dynamicOffset(ring_frames - 1),
        separated,
    });
